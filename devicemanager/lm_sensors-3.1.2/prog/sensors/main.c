...
/*
		...
		for(i=0;i<=socket_data_count;i++){
			printf("%s%+6.2f%s\n",socket_data[i].label,socket_data[i].data,socket_data[i].sulable);
		}
		printf("....................\n");
*/
	return cnt;
...
//		{"data_value",no_argument , NULL, 'd'},
	...
	data_value = 0;
	while (1) {
	c = getopt_long(argc, argv, "dhsvfAc:u", long_opts, NULL);
	...
	if (do_bus_list) {
		...
		if(!data_value){				
			...
			if(!socket_proc(do_the_real_work1)){
				printf("ERROR123!\n");		
			}	           
