...
void print_chip_raw(const sensors_chip_name *name)
{
	...
	int len;
	sensors_socket_data *socket_temp;
	...
	len= strlen(label)+1;
	sprintf(socket_temp->label,"%s", label);
...
//	printf("%s: \n", socket_data[socket_data_count].label);
//	printf("%d\n",socket_data_count);
//		 printf("break3\n"); 
	...
	if (sf && get_value(name, sf)) {
		...
		if (sf) {
			...
			socket_data_count++;//	printf("\n%d\n",socket_data_count);
			sprintf(socket_data[socket_data_count].label,"%s: ", label);
			oval=(float)val;
			socket_data[socket_data_count].data = oval;
			sprintf(socket_data[socket_data_count].sulable,"%s", degstr);
//			printf("data:%f: \n", socket_data[socket_data_count].data);
//			printf("break4\n"); 

/*			

			socket_temp->data = val;
			socket_insert(&socket_temp);
			printf("***%s",socket_data->label);
		      printf("#### %s",socket_data->data);
			socket_data_count++;
			*/
...
//	printf("break5\n"); 
	...
	if (sf){
		...
		oval=(float)get_value(name,sf);
		socket_data[socket_data_count].data = oval;
		sprintf(socket_data[socket_data_count].sulable,"%s", " V");
//		printf("data:%f: \n", socket_data[socket_data_count].data);
//		printf("break6\n"); 
...
//	printf("break7\n"); 
	...
	if (sf && get_value(name, sf))
		...
		if (sf){
			...
			oval=(float)get_value(name,sf);
			socket_data[socket_data_count].data = oval;
			sprintf(socket_data[socket_data_count].sulable,"%s", " RPM");
	//		printf("data:%f: \n", socket_data[socket_data_count].data);
	//		printf("break8\n"); 
...
//	printf("break9\n"); 
	...
	if (sf) {
		...
		oval=(float)val;
		socket_data[socket_data_count].data = oval;
		sprintf(socket_data[socket_data_count].sulable,"%s", " W");
//		printf("data:%f: \n", socket_data[socket_data_count].data);
//		printf("break10\n"); 
	} else
		printf("     N/A");
...
//	printf("break11\n"); 
	...
	if (sf) {	
		...
		oval=(float)val;
		socket_data[socket_data_count].data = oval;
		sprintf(socket_data[socket_data_count].sulable,"%s"," J");
...
//	sensors_socket_data *socket_temp;
	...
	if ((label = sensors_get_label(name, feature))
	 && !sensors_get_value(name, subfeature->number, &vid)) {
		...
		sprintf(socket_data[socket_data_count].label,"%s: ", label);
		ovid=(float)vid;
		socket_data[socket_data_count].data = ovid;
		sprintf(socket_data[socket_data_count].sulable,"%s", " V");
				   ...
				   int label_size)
{
	...
	if ((label = sensors_get_label(name, feature))
	 && !sensors_get_value(name, subfeature->number, &beep_enable)) {
		...
		socket_data_count++;
		sprintf(socket_data[socket_data_count].label,"%s: ", label);
		socket_data[socket_data_count].data = -1;
		sprintf(socket_data[socket_data_count].sulable,"%s",  beep_enable ? "enabled" : "disabled");
	}
	free(label);
}
...
//	sensors_socket_data *socket_temp;
	...
	if (sf){
		...
		oval=(float)get_value(name,sf);
		socket_data[socket_data_count].data = oval;
		sprintf(socket_data[socket_data_count].sulable,"%s"," A");
